...
init_jsi
let
    global init_jsi, default_language, type_languages, modelname_default, command, command_names, model, noises, noises_names, recognizer, controller, set_controller, do_perf_debug, use_static_recognizers, update_commands, use_tts
    ...
    function create_recognizer(backend::Symbol, modelname::String)::Recognizer
        ...
        elseif backend==:RealtimeSTT
            ...
            modelsize = (lang == "en-us") ? "tiny.en" : "tiny"
            r = RealtimeSTT.AudioToTextRecorder(post_speech_silence_duration=2.0, min_length_of_recording=0.0, min_gap_between_recordings=0.0, model=modelsize, use_main_model_for_realtime=true, language=lang, beam_size=10, allowed_latency_limit=10000, use_microphone=false, spinner=false)
    ...
    function init_jsi(commands::Dict{String, <:Any}, modeldirs::Dict{String, String}, noises::Dict{String, <:AbstractArray{String}}; default_language::String=LANG.EN_US, type_languages::AbstractArray{String}=[LANG.EN_US], vosk_log_level::Integer=-1, stt_freespeech_engine::String=STT_DEFAULT_FREESPEECH_ENGINE)
        # Set global options.
        ...
        for lang in type_languages
            if !haskey(modeldirs, modelname(MODELTYPE_SPEECH, lang)) && (stt_freespeech_engine == "vosk") @ArgumentError("a model directory for each selected type model is mandatory (entry \"$(modelname(MODELTYPE_SPEECH,lang))\" for type language $(lang_str(lang)) is missing).") end
        ...
        if modeldir == DEFAULT_VOSK_MODELDIRS[_modelname_default]
            if !isdir(modeldir)
                ...
                @voiceinfo "No model for the default language ($(lang_str(default_language))) found in its default location ($(modeldir)): downloading small model ($filename) from '$DEFAULT_MODEL_REPO' (~30-70 MB)..."
                ...
                if modeldir == DEFAULT_VOSK_MODELDIRS[modelname_lang]
                    if !isdir(modeldir)
                        if modeldir == ""
                            ...
                            if !isdir(modeldir)
                                filename = basename(modeldir) * ".zip"
                                @voiceinfo "No small model for $(lang_str(lang)) found in its default location ($(modeldir)): downloading small model ($filename) from '$DEFAULT_MODEL_REPO' (~30-70 MB)..."
                                modeldepot = joinpath(modeldir, "..")
                                download_and_unzip(modeldepot, filename, DEFAULT_MODEL_REPO)
                            end
                        else
                            filename = basename(modeldir) * ".zip"
                            @voiceinfo "No accurate large model for the type language ($(lang_str(lang))) found in its default location ($(modeldir)): download (optional) accurate large model ($filename) from '$DEFAULT_MODEL_REPO' (~1-2 GB)?"
                            answer = ""
                            ...
                            if answer == "yes"
                                ...
                                if modeldir == DEFAULT_VOSK_MODELDIRS[modelname_lang]
                                    if !isdir(modeldir)
                                        filename = basename(modeldir) * ".zip"
                                        @voiceinfo "No small dynamic model for the type language ($(lang_str(lang))) found in its default location ($(modeldir)): downloading small model ($filename) from '$DEFAULT_MODEL_REPO' (~30-70 MB)..."
                                        modeldepot = joinpath(modeldir, "..")
                                        download_and_unzip(modeldepot, filename, DEFAULT_MODEL_REPO)
                                    end
                                end
        ...
        for modelname in keys(modeldirs)
            if (stt_freespeech_engine == "vosk") && !isdir(modeldirs[modelname]) #TODO: see if this also should be done for whisper
                tilde_errmsg = ""
                if startswith(modeldirs[modelname], "~") tilde_errmsg = " Tildes ('~') in strings are not expanded for portability reasons (consider using 'homedir()')." end
                @ArgumentError("directory $(modeldirs[modelname]) does not exist.$tilde_errmsg")
            end
